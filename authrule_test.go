...
func TestAuthRuleNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.AuthRules.New(context.TODO(), lithic.AuthRuleNewParams{
		...
		AllowedCountries: lithic.F([]string{"CAN", "USA"}),
...
func TestAuthRuleMigrateV1ToV2(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.AuthRules.MigrateV1ToV2(context.TODO(), "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
	if err != nil {
		var apierr *lithic.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
