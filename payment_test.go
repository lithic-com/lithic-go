...
func TestPaymentNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Payments.New(context.TODO(), lithic.PaymentNewParams{
		...
		MethodAttributes: lithic.F(lithic.PaymentNewParamsMethodAttributes{
			...
			OverrideCompanyName: lithic.F("override_company_name"),
		}),
...
func TestPaymentRetryWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
...
func TestPaymentReturnWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
